...
class FrameDataSet(Dataset):
    def __init__(self, args, pattern='supervised_train', augmentation='supervised_train'):
        self.args = args
    ...
    def __load_file(self, pre="dataset", pattern='supervised_train', fold_num=0):
        if pattern == 'self_supervised_train':
            file_name = 'self_train_frame.xlsx'
        elif pattern in ['supervised_train', 'finetuning', 'distilling']:
...
class TiffDataSet(Dataset):
    def __init__(self, args, pattern='supervised_train', augmentation='supervised_train', fold_num=0):
        ...
        # 判断是否将所有的帧数据直接加载到内存中
        if self.args.load_in_memory:
            self.imgs = [TiffFile(img_path) for img_path in self.imgs]
        self.img_processor = ImageProcessor(args, augmentation=augmentation)

    def __getitem__(self, index):
        imgPath = self.imgs[index]
        label = self.labels[index]
        tif = imgPath if self.args.load_in_memory else TiffFile(imgPath)
        ...
        for f_idx in range(image_arr.shape[0]):
            ...
            ith_frame = self.img_processor(ith_frame)  # [3, H, W]
    ...
    def __load_file(self, pre="data_folder", pattern='supervised_train', fold_num=0):
        if pattern in ['supervised_train', 'finetuning', 'distilling']:
...
if __name__ == '__main__':
    ...
    imgPath = '/ssd_data/dataset/tiff_xinhua_1_frame/囊肿/M0008_2021_P0000062_circle_2.0x2.5_C6_S6_3.png'
    imgPath_split = imgPath.split('/')
    imgPath_split.insert(4, 'segmentation_mask')  #
    mask_path = '/'.join(imgPath_split)
    print(mask_path)
